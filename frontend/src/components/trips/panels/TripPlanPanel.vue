...
import { fetchTripItineraryFull, fetchSavedTripItinerary } from "@/composables/api/itinerary.js"
...
async function loadSavedItinerary() {
  ...
  try {
    const res = await fetchSavedTripItinerary(props.tripId)
    const dto = res?.data?.data || null
    if (dto?.schedule?.length) setItineraryData(dto)
    else clearItinerary()
  } catch (err) {
...
async function generateRoute() {
  ...
  try {
    const payload = { days: Number(days.value || 1) }
...
const drag = ref({ dayIndex: null, fromIndex: null, overIndex: null })
...
watch(
    () => [props.placesLoading, props.places.length],
    () => {
      loadAiSuggestions()
    }
)
...
<template>
  <section class="bg-white p-4 sm:p-6 rounded-2xl border shadow-sm">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      ...
      <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
        <button
            type="button"
            class="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium border border-gray-200 bg-white text-gray-900 hover:bg-gray-50 transition disabled:opacity-50 disabled:cursor-not-allowed"
            :disabled="generating || loadingSaved"
            @click="loadSavedItinerary"
        >
        ...
        <button
            type="button"
            class="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:opacity-90 active:opacity-80 shadow disabled:opacity-50 disabled:cursor-not-allowed"
            :disabled="!canGenerate || generating"
            @click="generateRoute"
        >
          <Sparkles class="h-4 w-4" />
          {{ generating ? tr("trip.plan.generating", "Generating...") : tr("trip.plan.generate_full", "Generate route") }}
        </button>
      </div>
    </div>

    <div class="mt-4 sm:mt-6 rounded-2xl border border-gray-200 p-4 sm:p-5">
      <div class="grid grid-cols-1 sm:grid-cols-12 gap-3">
        ...
        <div class="sm:col-span-8 flex items-end">
          <div v-if="!canGenerate" class="w-full p-3 rounded-xl border bg-gray-50 text-gray-700">
            <span class="font-semibold">{{ tr("trip.plan.empty_title", "Add places first") }}</span>
            <span class="text-sm text-gray-600 ml-2">{{ tr("trip.plan.empty_hint", "At least one place is required.") }}</span>
          </div>

          <div v-else-if="!itinerary && !generating && !loadingSaved" class="w-full p-3 rounded-xl border bg-gray-50 text-gray-700">
            <span class="font-semibold">{{ tr("trip.plan.ready_title", "No route yet") }}</span>
            <span class="text-sm text-gray-600 ml-2">{{ tr("trip.plan.ready_hint", "Choose days and click Generate route.") }}</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
          <input
              ...
              min="1"
              max="20"
              class="w-full sm:w-20 h-10 px-3 rounded-xl border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-black/10 text-sm"
          />
          <button
              type="button"
              class="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium border border-gray-200 bg-white text-gray-900 hover:bg-gray-50 transition disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    <div v-if="placesLoading" class="mt-4 sm:mt-6 p-5 rounded-xl border bg-gray-50 text-gray-700">
      {{ tr("loading", "Loading...") }}
    </div>

    <div v-else-if="itinerary" class="mt-4 sm:mt-6 space-y-4">
      <div class="rounded-2xl border border-gray-200 p-5">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div class="font-semibold">{{ tr("trip.plan.generated_title", "Plan generated") }}</div>
            <div class="text-sm text-gray-600 mt-2">
              {{ tr("trip.plan.days_label", "Days") }}:
              <span class="font-semibold text-gray-900">{{ itinerary.day_count }}</span>
            </div>
          </div>
      ...
      <div
          v-for="(day, dayIndex) in itinerary.schedule"
          :key="day.day"
          class="rounded-2xl border border-gray-200 overflow-hidden"
      >
        <div class="px-5 py-4 bg-gray-50 border-b border-gray-200">
          <div class="flex items-center justify-between gap-3">
            <div class="font-semibold">{{ tr("trip.plan.day", "Day") }} {{ day.day }}</div>
            <div class="text-sm text-gray-600">
              {{ tr("trip.plan.places", "Places") }}:
              <span class="font-semibold text-gray-900">{{ day.places?.length || 0 }}</span>
            </div>
          </div>
        </div>
            ...
            <div class="min-w-0 flex-1">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="text-sm text-gray-700">
                  {{ tr("trip.plan.score", "Score") }}:
                  <span class="font-semibold">{{ formatScore(p.score) }}</span>
                </span>

                <span class="text-xs text-gray-400">•</span>

                <span class="text-sm text-gray-700">
                  {{ tr("trip.plan.distance", "Distance") }}:
                  <span class="font-semibold">{{ formatDistance(p.distance_m) }}</span>
                </span>
              </div>

              <div class="mt-1 text-xs text-gray-500">#{{ p.id }}</div>
            </div>
          </li>
        </ul>
      </div>
