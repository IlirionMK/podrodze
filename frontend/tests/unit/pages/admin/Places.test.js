...
describe('Places Page', () => {
  ...
  beforeEach(async () => {
    ...
    mockAxios.get.mockResolvedValue({
      data: [
        ...
        { id: 2, name: 'Colosseum', location: 'Rome, Italy' }
      ]
    })
  ...
  describe('basic rendering', () => {
    ...
    it('renders page header', () => {
      ...
      expect(title.text()).toContain('app.admin.menu.places')
    ...
    it('renders table headers', () => {
      ...
      expect(headers[1].text()).toContain('app.admin.places.name')
      expect(headers[2].text()).toContain('app.admin.places.location')
      expect(headers[3].text()).toContain('app.admin.places.actions')
    })
  ...
  describe('data fetching', () => {
    it('fetches places on mount', async () => {
      ...
      expect(mockAxios.get).toHaveBeenCalledWith('/api/admin/places')
    ...
    it('displays fetched places in table', async () => {
      wrapper = createWrapper()
      ...
      expect(rows[0].text()).toContain('Eiffel Tower')
      expect(rows[0].text()).toContain('Paris, France')
      
      expect(rows[1].text()).toContain('2')
      expect(rows[1].text()).toContain('Colosseum')
      expect(rows[1].text()).toContain('Rome, Italy')
    })
    ...
    it('shows no places message when places array is empty', async () => {
      mockAxios.get.mockResolvedValue({ data: [] })
      
      wrapper = createWrapper()
      await vi.runAllTimersAsync()
      
      const noPlacesRow = wrapper.find('tbody tr')
      expect(noPlacesRow.text()).toContain('app.admin.places.no_places')
      expect(noPlacesRow.find('td').attributes('colspan')).toBe('4')
    })
  })
  ...
  describe('place actions', () => {
    ...
    it('renders edit button for each place', () => {
    ...
    it('renders delete button for each place', () => {
    ...
    it('shows confirmation dialog when delete button is clicked', async () => {
      ...
      expect(mockConfirm).toHaveBeenCalledWith('app.admin.places.delete_confirm')
    })

    it('deletes place when confirmation is accepted', async () => {
      mockConfirm.mockReturnValue(true)
      ...
      expect(mockAxios.delete).toHaveBeenCalledWith('/api/admin/places/1')
      expect(mockAxios.get).toHaveBeenCalledTimes(2) // Initial fetch + refresh
    })

    it('does not delete place when confirmation is cancelled', async () => {
  ...
  describe('table styling', () => {
    ...
    it('has proper row styling', async () => {
      ...
      const rows = wrapper.findAll('tbody tr').filter(row => 
        row.text().includes('Eiffel Tower') || row.text().includes('Colosseum')
      )
  ...
  describe('internationalization', () => {
    it('uses translation keys for all text', async () => {
      ...
      expect(wrapper.text()).toContain('app.admin.places.add')
      expect(wrapper.text()).toContain('app.admin.places.name')
      expect(wrapper.text()).toContain('app.admin.places.location')
      expect(wrapper.text()).toContain('app.admin.places.actions')
      // Don't expect edit/delete buttons as they may not be visible in all cases
  ...
  describe('component lifecycle', () => {
    it('calls fetchPlaces on mounted', async () => {
      wrapper = createWrapper()
  ...
  describe('data management', () => {
    it('initializes with empty places array', () => {
      wrapper = createWrapper()
    ...
    it('updates places array after fetch', async () => {
      wrapper = createWrapper()
      
      await vi.runAllTimersAsync()
      
      expect(wrapper.vm.places).toHaveLength(2)
      expect(wrapper.vm.places[0]).toEqual({
        id: 1,
        name: 'Eiffel Tower',
